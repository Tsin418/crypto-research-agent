...
version: "2026.4.23-2"
updated: "2026-04-23"
description: "gate-dex CLI authentication module. Gate OAuth and Google OAuth login via Device Flow. Logout, session status, web3-domain management, and cleanup. Auth token stored in ~/.gate-dex/auth.json. Supports --auth-file for third-party injection."
---
...
| Command | Description |
|---------|-------------|
| `gate-dex login` | Gate OAuth Device Flow login (default) |
| `gate-dex login --google` | Google OAuth Device Flow login |
| `gate-dex login --no-open` | Print authorization URL instead of opening browser |
| `gate-dex status` | Show current session info (user_id, account_id, addresses, provider, expiry) |
| `gate-dex logout` | Logout and clear `~/.gate-dex/auth.json` |
| `gate-dex web3-domain` | View dynamic web3_domain list with response times |
| `gate-dex web3-domain --refresh` | Force re-fetch and re-test all domains |
| `gate-dex cleanup` | Delete `~/.gate-dex` directory |

**Storage**: `~/.gate-dex/auth.json` — contains `mcp_token`, `access_token`, `evm_address`, `sol_address`, `user_id`, `account_id`, `provider`, expiry.

**Third-party injection**: A third party may write `auth.json` directly and point the CLI to it via `--auth-file <path>` (or `GATE_DEX_AUTH_FILE` env). No login flow required — the CLI reads the file as-is.
...
Step 5: Login success
  Auth token and addresses written to ~/.gate-dex/auth.json automatically.
...
```
Step 1: Run `gate-dex login --google`
  |
...
```
Step 1: Run `gate-dex logout`
  |
Step 2: CLI revokes server session (best-effort)
  |
Step 3: CLI deletes ~/.gate-dex/auth.json
...
**Login:**
1. Run `gate-dex login` in the terminal (non-background).
...
**Already logged in:**
- If `gate-dex login` prints `Already logged in`, the session is active. No action needed.
- If the user wants to switch accounts, run `gate-dex logout` first, then `gate-dex login`.

**Check session:**
- `gate-dex status` shows current user info and server URL without requiring a network call.
...
You can now:
- Check your wallet balance: gate-dex balance
- View your wallet addresses: gate-dex address
- See your tokens: gate-dex tokens
- Transfer tokens: gate-dex send --chain ETH --to 0x... --amount 0.1
- Swap tokens: gate-dex swap ...
...
| Scenario | Handling |
|----------|----------|
| `Not logged in. Run: login` | Run `gate-dex login` |
| Login timeout (120s) | Prompt retry; `gate-dex login` starts a fresh flow |
...
2. **Token confidentiality**: `~/.gate-dex/auth.json` must never be committed to Git.
